...
var subscriptionService = function(io){
  ...
  this.handleIncomingConnection = function(connection) {
    ...
    connection.on('itemBought', function(data){
      debug('item bought');
      debug(data);
      Item.setAsBought(data.id, data.status, function (err, item) {
        if (err) {
          debug(err);
        } else {
          connection.emit('changedBoughtStatus', item);
        }
      });
      //Item.find({id:data.id}).execute(function(err, item){
      //  if(err){
      //    debug(err);
      //  } else {
      //    Item.
